// FLUIDOS
 var liquido = { uno: 51, dos: 155, tres: 223, cuatro: 130 };
 var fluido = "acetona";

// Variables del cubo
var cubox = 40, cuboy = 40, cuboz = 40, imagen = "madera.jpg";
...
var iniciar = 0, img, andar = 0, objeto = "cubo", efecto = 0, contador = 0, aparecer = 0, mostrar = 0;

var pg /* valor restante */, pg2 /* calado */, pg3, pg4, pg5, pg6;

var gravedad = parseFloat("9.81"), v, calado, resto, masa, densidad, kn = 0.001, volumen_cubo, area_cubo, alt, area_cilindro, volumen_cilindro, 
mostrar_v_m = 0, boton, reinicio = 1, distancia, pxcalado, calculo_cubo, agua_peso = 9.81, imagen, caida, apagar = 0;
...
function preload(){
  ...
  // Cargar imagenes de las figuras

  img = loadImage(imagen);

  //Cargar Sonido

  plash = loadSound("sonidos/plash.mp3");
  plash2 = loadSound("sonidos/plash2.mp3");
  fondo = loadSound("sonidos/fondo.mp3");
  fondo2 = loadSound("sonidos/fondo2.mp3");
  denso = loadSound("sonidos/mercurio.mp3");
...
var resto_boton = windowHeight * 13 / 100; 
...
boton.style("padding-left","2.5%");
boton.style("background","url('imagenes/interact.png')"); boton.style("background-position","4%"); boton.style("background-size","25%");
boton.style("background-repeat","no-repeat");
boton.style("background-color","#428bca"); boton.style("margin-bottom","0"); 
boton.style("display","inline-block"); boton.style("font-size","135%"); boton.style("font-weight","normal");
boton.style("line-height","1.42857143"); boton.style("text-align","center"); boton.style("white-space","nowrap");
boton.style("vertical-align","middle"); boton.style("border","1px solid transparent"); boton.style("border-radius","4%");
boton.style("color","#FFFFFF");

boton.mousePressed(reiniciar); 


} function reiniciar(){ javascript:location.reload(); } //FUNCTION PARA RECARGAR SIMULADOR
...
botonc.style("padding","1%");
botonc.style("padding-left","2.5%");
botonc.style("background","url('imagenes/close.png')"); botonc.style("background-position","4%"); botonc.style("background-size","25%");
botonc.style("background-repeat","no-repeat");
botonc.style("background-color","#d9534f"); botonc.style("margin-bottom","0"); 
botonc.style("display","inline-block"); botonc.style("font-size","135%"); botonc.style("font-weight","normal");
botonc.style("line-height","1.42857143"); botonc.style("text-align","center"); botonc.style("white-space","nowrap");
botonc.style("vertical-align","middle"); botonc.style("border","1px solid transparent"); botonc.style("border-radius","4%");
botonc.style("color","#FFFFFF");

} function cerrar(){ window.close(); } //FUNCTION PARA CERRAR SIMULADOR

//****************************

if(apagar == 0){ //FUNCTION PARA CREAR SONIDO DEL SIMULADOR

botons = createButton("Sonido"); botons.position(windowWidth * 89 / 100,windowHeight - largo_pantalla - resto_boton); 
botons.mousePressed(off); 
botons.style("padding","1%");
botons.style("padding-left","2.5%");
botons.style("background","url('imagenes/sound.png')"); botons.style("background-position","4%"); botons.style("background-size","25%");
botons.style("background-repeat","no-repeat");
botons.style("background-color","#31708f"); botons.style("margin-bottom","0"); 
botons.style("display","inline-block"); botons.style("font-size","135%"); botons.style("font-weight","normal");
botons.style("line-height","1.42857143"); botons.style("text-align","center"); botons.style("white-space","nowrap");
botons.style("vertical-align","middle"); botons.style("border","1px solid transparent"); botons.style("border-radius","4px");
botons.style("color","#FFFFFF"); 

} function off(){  //APAGAR

  botons.style("background","url('imagenes/speaker.png')"); botons.style("background-position","4%"); botons.style("background-size","25%"); 
  botons.style("background-repeat","no-repeat");
  botons.style("background-color","#31708f"); botons.style("margin-bottom","0"); 
  botons.mousePressed(on);

  apagar = 1;

}

function on(){ //ENCENDER
...
select_fluidos.position(windowWidth * 1 / 100,10);
select_fluidos.size(windowWidth * 8 / 100);
...
select_material.position(windowWidth * 10 / 100,10);
select_material.size(windowWidth * 8 / 100);
select_material.id("idmaterial");
select_material.option("Material:"); select_material.option("madera.jpg"); select_material.option("acero.jpg"); select_material.option("aluminio.jpg");
select_material.option("corcho.jpg"); select_material.option("hierro.jpg"); select_material.option("bronce.jpg"); select_material.option("cobre.jpg");
select_material.option("plata.jpg"); select_material.option("plomo.jpg"); select_material.option("oro.jpg"); select_material.option("platino.jpg");
select_material.changed(mimaterial);

//figura
select_objeto = createSelect();
select_objeto.position(windowWidth * 18.7 / 100,10);
select_objeto.size(windowWidth * 6 / 100);
select_objeto.id("idfigura");
select_objeto.option("Figuras:"); select_objeto.option("cilindro"); select_objeto.option("cubo");
select_objeto.changed(mifigura);

//iniciar

botones = createButton('Iniciar'); 
botones.position(windowWidth * 40 / 100,10);
...
function mifigura()   { objeto = select_objeto.value(); 

if(objeto == "cilindro") {

//Dimensiones
select_cilx = createSelect();
select_cilx.position(windowWidth * 26 / 100,10);
select_cilx.size(windowWidth * 6 / 100);
...
select_cily.position(windowWidth * 33 / 100,10);
select_cily.size(windowWidth * 6 / 100);
...
//Dimensiones
select_cux = createSelect();
select_cux.position(windowWidth * 26 / 100,10);
select_cux.size(windowWidth * 6 / 100);
select_cux.id("idcubo");
select_cux.option("Lados:"); select_cux.option(20); select_cux.option(30); select_cux.option(40);
                            select_cux.option(50); select_cux.option(60);
select_cux.changed(dimension2);
...
document.getElementById("idcubo").disabled = true; }
...
function draw(){
 ...
 var alto_pasto = 100 * windowHeight / 638, y_pasto = 34 * windowHeight / 638,
     ...
     pg3_ftx = -530 * windowWidth / 1366, pg3_fty = -220 * windowHeight / 638, pg3_fpx = 230 * windowWidth / 1366, pg3_fpy = 200 * windowHeight / 638,
     ...
     p5_dtx = 600 * windowWidth / 1366, p5_dty = -240 * windowHeight / 638, p5_dpx = 200 * windowWidth / 1366, p5_dpy = 130 * windowHeight / 638,
...
var tx_barra = 40 * windowWidth / 1366;
  ...
  //******************************************************************* IMAGENES CAMBIO **********************************************************
    ...
    if(imagen == "madera.jpg") { densidad = 600 } if(imagen == "acero.jpg") { densidad = 7800 } if(imagen == "aluminio.jpg") { densidad = 2700 }

    if(imagen == "corcho.jpg") { densidad = 240 } if(imagen == "hierro.jpg") { densidad = 7860 } if(imagen == "bronce.jpg") { densidad = 8600 }

    if(imagen == "cobre.jpg") { densidad = 8900 } if(imagen == "plata.jpg") { densidad = 10500 } if(imagen == "plomo.jpg") { densidad = 11340 }

    if(imagen == "oro.jpg") { densidad = 19300 } if(imagen == "platino.jpg") { densidad = 24400 }
...
//CALCULAR VOLUMEN Y AREA DEL CUBO

v = cubox / 10;
area_cubo = v*v;
volumen_cubo = v*v*v;

masa = densidad * volumen_cubo; //calcular masa del objeto

calado = (masa * gravedad * kn) / (liquido.gravedad_es * agua_peso * area_cubo); // calcular calado

resto = v - calado; //calcular resto
...
if(iniciar == 1){ 
  ...
  mostrar_v_m = 1;
  //Si el material esta hecho de madera o corcho, usamos un sonido de agua mas suave
  if(imagen == "madera.jpg" || imagen == "corcho.jpg"){
    if(apagar == 0){
  plash.setVolume(0.7);

  if(fluido != "mercurio"){

  plash.play(); } else { denso.play(); } }
}
  //Si no, usamos un sonido mas fuerte
  else{
    if(apagar == 0){

      if(fluido != "mercurio"){
  plash2.play(); } else { denso.play(); }  }
}
...
pxcalado = calado * pantalla_calado; //POSICION DE BARRA DE CALADO

pxrestante = resto * pantalla_resto; //POSICION BARRA DE VALOR RESTANTE

//************************************* CUBO SUMERGIDO

if(iniciar == 1 && calado < v) //SI EL CALADO O DISTANCIA SUMERGIDA ES MENOR AL VOLUMEN DEL OBJETO
...
if(p_cuboy = calculo_cubo){ //UBICAR OBJETO EN POSICION SUMERGIDA
...
else if (iniciar == 1 && calado > v) //SI EL CALADO O VOLUMEN SUMERGIDO ES MAYOR AL VOLUMEN DEL OBJETO
...
v = cilindroy / 10;
area_cilindro =  3.14 * cilindrox / 10 * cilindrox / 10;
volumen_cilindro = 3.14 * cilindrox / 10 * cilindrox / 10 * v;

masa = densidad * volumen_cilindro;

calado = (masa * gravedad * kn) / (liquido.gravedad_es * agua_peso * area_cilindro);
...
var pantalla_resto = 10 * windowHeight / 638, tcubo_pxr = resto * pantalla_resto;
    pantalla_calado = 10 * windowHeight / 638, tcubo_pxc = calado * pantalla_calado;

//CALADO MENOR AL VOLUMEN DEL OBJETO - SI EL OBJETO FLOTA

if(iniciar == 1 && calado < v)
...
pxcalado = calado * pantalla_calado; //CONVERTIR CALADO DE METROS A PIXELES

pxrestante = resto * pantalla_resto; //CONVERTIR RESTO DE METROS A PIXELES
...
pg5.text("Masa: " + masa.toFixed(4) + " kg", 5, 180);
    texture(pg5);
    plane(p5_opx,p5_opy);
...
//PESO ESPECIFICO DEL FLUIDO Y DEL MATERIAL
...
push();
translate(0,y_ftanque + ftanquey,0);
fill("#FFFFFF");
plane(100,1);
pop();*/
